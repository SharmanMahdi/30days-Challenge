//function to show and hide the password in the input
function showHidePassword(isHidden, val) {
    var password = (val === 0) ? document.getElementById('inputPassword') : document.getElementById('inputPasswordConf')
    var showHide = (val === 0) ? document.getElementById('show-hide') : document.getElementById('show-hide-conf')
    if (isHidden) {
        password.type = "text"
        showHide.innerHTML = `<i class="bi bi-eye" onclick="showHidePassword(false, ${val})"></i>`
    } else {
        password.type = "password"
        showHide.innerHTML = `<i class="bi bi-eye-slash" onclick="showHidePassword(true, ${val})"></i>`
    }
}
